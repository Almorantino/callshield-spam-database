# CallShield Live Caller ID PIR Service

This directory is the isolated Apple-native backend foundation for CallShield Live Caller ID.
It is based on Apple's official `pir-service-example` reference implementation and is kept separate
from the Cloudflare Worker that serves Message Filter and Reporting.

Initial CallShield scope:
- keep the existing Worker JSON routes untouched;
- expose the Apple-required PIR endpoints `/config`, `/key`, and `/queries`;
